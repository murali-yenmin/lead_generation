...
import {
  ...
  setSocialPostsPage,
  setSocialPostsLimit,
} from "@/store/slices/socialPostsSlice";
...
function Filters({ className }: { className?: string }) {
  const dispatch = useDispatch<AppDispatch>();
    const { platformFilter, dateRange: dateRangeStrings } = useSelector((state: RootState) => state.socialPosts);
  ...
  return (
    <div className={cn("flex flex-col sm:flex-row gap-4", className)}>
      <Select
        ...
        onValueChange={(value) => dispatch(setPlatformFilter(value))}
      >
...
export default function SocialMediaAnalyticsPage() {
  ...
  const {
    ...
    page,
    limit,
  } = useSelector((state: RootState) => state.socialPosts);
  ...
  }, [dispatch, token, platformFilter, dateRange, page, limit]);
  ...
  const columns = useMemo<ColumnDef<SocialPost>[]>(
    () => [
          ...
          return (
            <div className="flex items-start gap-4">
              <div className="size-16 rounded-md overflow-hidden shrink-0 bg-muted">
                <Image
                  src={
                    imageUrl || "https://placehold.co/64x64.png?text=No+Image"
                  }
                  alt="Post image"
                  width={64}
                  height={64}
      ...
      },
      {
        id: "impressions",
        header: "Impressions",
        cell: ({ row }) => row.original.analytics.impressions.toLocaleString(),
      },
      {
        id: "reach",
        header: "Reach",
        cell: ({ row }) => row.original.analytics.reach.toLocaleString(),
      },
      {
        id: "likes",
        header: "Likes",
        cell: ({ row }) => row.original.analytics.likes.toLocaleString(),
      },
      {
        id: "comments",
        header: "Comments",
        cell: ({ row }) => row.original.analytics.comments.toLocaleString(),
      },
      {
        id: "shares",
        header: "Shares",
        cell: ({ row }) => row.original.analytics.shares.toLocaleString(),
      },
      {
        id: "clicks",
        header: "Clicks",
        cell: ({ row }) => row.original.analytics.clicks.toLocaleString(),
      },
      {
        id: "saves",
        header: "Saves",
        cell: ({ row }) => row.original.analytics.saves.toLocaleString(),
      },
      {
        id: "engagementRate",
        header: "Eng. Rate",
        cell: ({ row }) => `${row.original.analytics.engagementRate}%`,
      },
    //   { id: 'leads', header: 'Leads', cell: ({ row }) => row?.original?.analytics?.leads?.toLocaleString() },
    ],
    []
  );
  ...
  return (
    <main className="flex flex-1 flex-col gap-6 p-4 sm:px-6 md:gap-8 md:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
         {/* <AnalyticsSummaryCard title="Total Leads" value={summary?.totalLeads || 0} icon={<Goal />} isLoading={isLoading} /> */}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A detailed breakdown of each post's analytics.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
            ...
            pageCount={pageCount}
            currentPage={page}
            onPageChange={(p) => dispatch(setSocialPostsPage(p))}
            currentLimit={limit}
            onLimitChange={(l) => dispatch(setSocialPostsLimit(l))}
            hidePagination={true}
          >
            <Filters />
          </DataTable>
